...
class StudentController < ApplicationController
  ...
  def schools
    @student_schools = School.find_all_by_is_deleted false
    @student_school = School.new(params[:student_school])
    if request.post? and @student_school.save
      flash[:notice] = "#{t('flash7')}"
  ...
  def school_edit
    @student_school = School.find(params[:id])
  end

  def school_delete
    if @student_school = School.update(params[:id], :is_deleted=>true)
      @student_school.empty_school
    end
    @student_schools = School.find_all_by_is_deleted false
  end
  ...
  def school_update
    @student_school = School.find(params[:id])
    @student_school.update_attribute(:school_name, params[:school_name])
    @student_schools = School.find_all_by_is_deleted false
  end
