...
#                               on a.id = b.course_id
#                            where b.id in (select a.batch_id
#				                             from subjects a
#					                              inner join employees_subjects b
  ...
  def current_school
    ...
    if @user.admin?
      ...
      @subjects = Subject.find(:all,
        #:select => "batch_id",
        :joins => "inner join employees_subjects b on b.subject_id = subjects.id",
        :conditions => "is_deleted = false "+
          "and b.employee_id = \'"+@employee.id.to_s+"\'")
      if @subjects.count > 0
         @courses = Course.find(:all,
          #:select => "courses.id",
          :joins => " inner join batches b on courses.id = b.course_id",
          :conditions => "courses.is_deleted = false "+
            "and b.is_active = true "+
            "and b.is_deleted = false " +
            "and b.id in ("+@subjects.collect { |e| e['batch_id'].to_s + ','  }.uniq.join.to_s+"-1)"
        )
      end
      if @courses.count > 0                    
         @schools = School.find(:all,
          :conditions => "is_deleted = false and id in ("+@courses.collect { |c| c['school_id'].to_s + ','  }.uniq.join.to_s+"-1)"
        )
      end

    elsif @user.parent or @user.student
          @student = Student.find(:all,
                                 #:select => "",
                                  :joins => "inner join batches b on students.batch_id = b.id ",
                                  :conditions => "b.is_active = true and b.is_deleted = false "+
